...
2. **Project scaffold** — repo init, framework choice (§3), Cloudflare deploy pipeline, a live "hello world" placeholder page. Proves the plumbing before building features on it. **Status: functionally complete, one real gap remaining.** Astro scaffolded, `.gitignore` and `.nvmrc` in place, README added, placeholder page builds from `data/races.json` at build time with zero shipped JS, licensing resolved (no license file — §8), live site verified in production matching the local build exactly (71 active / 75 total / 75 with coordinates / 14 marquee). **Remaining:** (1) confirm push-to-deploy actually works end-to-end — a local commit changing visible footer text is sitting unpushed as the test case, push it and confirm the live site updates; (2) **deploy config isn't version-controlled** — build command, output directory, and assets binding currently live only in the Cloudflare dashboard, invisible to a future session, and `wrangler deploy` from a clean clone wouldn't work without it. Needs a `wrangler.jsonc`/`wrangler.toml` committed to the repo matching the dashboard settings — flagged by Claude Code, not yet done, worth treating as a real Phase 2 blocker rather than deferred debt.
