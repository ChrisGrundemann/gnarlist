...
7. **Deferred-item cleanup pass** — the six items the design sweep left flagged, closed out together rather than allowed to accumulate into a seventh. Three were real defects, one was real but misdiagnosed, one was not a defect at all, and one had been deferred three times for no reason anybody had checked. **Status: built and verified; the user's visual confirmation is still the last word (§1).**
   ...
   **Noticed while in here, deliberately not fixed** (outside this pass's defined list):
   - **`.rgroup.is-empty` on the map index uses `opacity: 0.62` on text** (`src/pages/map.astro`), the exact treatment §2.5's corollary forbids and that `CalendarMonth.astro` already retired for the same reason. Reachable whenever a filter empties a region: "No events" and the count measure **3.00:1** and **3.93:1** — real AA failures the audit misses because it only samples the unfiltered state. The calendar's fix (quieter ink, no opacity) ports directly.
   - **§2.5 and the code disagree about `unverified`.** §2.5 names rust as the single warning ink "(discontinued/unverified)", but every view draws unverified in dashed `--gold-deep`. The code's idiom reads well as "uncertainty"; the doc should be amended to match, or the code changed. Zero unverified events today, so nothing is visibly wrong.
