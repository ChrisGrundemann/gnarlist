...
4. **Calendar view + event permalink pages** — interactive, filterable calendar generalizing the static poster, plus a real linkable page per event (pulled forward from Phase 6, see §4.6 — both the list and calendar views link to these pages). **Status: built, pending the user's visual confirmation.** `/calendar/` renders all 102 events in twelve month blocks; `/races/<slug>/` renders the full record for each of the 102. `astro.config.mjs`'s `site` now reads `https://gnarlist.co`, so canonical/OG URLs point at the canonical domain rather than the `workers.dev` one.
   ...
   **How these were actually implemented in Phase 4** (appended per the documentation convention above; §4's Phase 3 note is the model):
   ...
   - **Month-density variance is handled by CSS multi-column, not a grid.** A row-major grid row is as tall as its tallest cell, so a 20-event September beside an empty March leaves a hole the height of nineteen races. `columns: 1 / 2 / 3` at 42rem and 68rem lets each block be exactly its own height and flows the next in behind it — the paste-up the poster did by hand — and it re-packs itself for free when a filter changes, with no measuring and no JS. Months stay in order reading down each column. **A month block is never removed**, even when a filter empties it: it collapses to a slim "No events" state so the year keeps its twelve-block shape. March is empty in the dataset and renders that way at rest, which is a fact about the Colorado season, not a gap.
   - **A density strip above the grid** doubles as the season profile and as mobile navigation — twelve bars scaled to the busiest month, each an anchor to its block, so reaching October on a phone doesn't mean scrolling past September's twenty events. Both the strip bars and each block's meter redraw against the *counted* number as filters change.
   ...
   **Known follow-ups from Phase 4, not blockers:**
   ...
   - No `sitemap.xml`. Now that `site` is correct and there are 104 indexable pages instead of one, `@astrojs/sitemap` is a genuinely cheap add — left out this session because it means a new dependency, which wasn't in scope.
   - No JSON-LD `Event` structured data on the permalinks. The title/description/canonical requirement is met; structured data is the obvious next SEO step and was not asked for.
